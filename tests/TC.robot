...
*** Settings ***
Library           AppiumLibrary    timeout=60    run_on_failure=Capture Page Screenshot    sleep_between_wait_loop=0.2
Test Setup        Open Application    remote_url=http://127.0.0.1:4723    platformName=ios    appium:automationName=xcuitest    appium:deviceName=iPhone 16    appium:platformVersion=18    appium:udid=5FBB2585-CE24-4954-ABE6-95B257451381    appium:noReset=${False}    appium:fullReset=${False}    appium:printPageSourceOnFindFailure=${False}    appium:bundleId=org.reactjs.native.example.wdiodemoapp    appium:app=${CURDIR}${/}..${/}resources${/}app${/}wdiodemoapp.app   appium:includeSafariInWebviews=${True}    appium:newCommandTimeout=${3600}    appium:connectHardwareKeyboard=${True}    disableIdLocatorAutocompletion=${True}
